...
export default function EditProductPage() {
    ...
    if (fetching) return <div className="flex justify-center p-10"><Loader2 className="animate-spin" /> Loading data...</div>;
    ...
    return (
        <div className="p-8 max-w-2xl mx-auto bg-white shadow-lg rounded-xl my-10 border border-slate-100">
           ...
           <div className="mb-6">
             ...
             <h1 className="text-3xl font-bold mb-6 text-slate-900">Edit Product #{id}</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <Input type="number" value={formData.stockLevel} onChange={e => setFormData({ ...formData, stockLevel: e.target.value })} required />
                    </div>
                </div>

                <div className="grid grid-cols-3 gap-4">
                    <select className="border p-2 rounded" value={formData.color} onChange={e => setFormData({ ...formData, color: e.target.value })}>
                        {COLORS.map(c => <option key={c.value} value={c.value}>{c.name}</option>)}
                    </select>
                    <select className="border p-2 rounded" value={formData.size} onChange={e => setFormData({ ...formData, size: e.target.value })}>
                        {SIZES.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    <select className="border p-2 rounded" value={formData.style} onChange={e => setFormData({ ...formData, style: e.target.value })}>
